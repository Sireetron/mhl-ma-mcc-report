...
data = df[df['Execution Date'].dt.month == 9]["Percent Success"]
print(data)
...
if len(dates) > 0 and len(data) > 0:
    ...
    cax = july.month_plot(dates, data, month=9, cmap=custom_cmap, ax=ax)
...
try:
    ...
    response_total_run = requests.get(api_url_total_run, params=api_params, headers=headers)
